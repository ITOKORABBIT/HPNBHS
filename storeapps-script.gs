...
function rowToStore(r) {
  return {
    ...
    photo10:    String(r[24] || ''),
    note:       String(r[25] || ''),
    lastUpdate: fmtDate(r[26]),
    reviewer:   String(r[27] || ''),
    pubName:    String(r[28] || ''),
    pubCate:    String(r[29] || ''),
    pubPhone:   String(r[30] || ''),
    pubAddr:    String(r[31] || ''),
    pubMapUrl:  String(r[32] || ''),
    pubDesc:     String(r[33] || ''),
    pubOffer:    String(r[34] || ''),
    pubHours:    String(r[35] || ''),
    pubStoreNum: String(r[36] || ''),
    planType:    String(r[37] || '免費'),
    pinOrder:    Number(r[38] || 0),
  };
}
